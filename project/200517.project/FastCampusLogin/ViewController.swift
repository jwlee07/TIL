...
class ViewController: UIViewController, UITextFieldDelegate {
  ...
  override func viewDidLoad() {
    ...
    loginList.set("qwqqw1332", forKey: "qwqqw1332")
  }
  ...
  override func viewWillLayoutSubviews() {
    ...
    signinButton.addTarget(self, action: #selector(actionSignInButoon), for: .touchUpInside)
    view.addSubview(signinButton)
    
    Text.email.delegate = self
  ...
  @objc func actionLoginButton(_ sender: UIButton) {
    if (Text.email.text?.count ?? 0 > 3 && Text.email.text?.count ?? 0 < 17),
      (Text.password.text?.count ?? 0 > 3 && Text.password.text?.count ?? 0 < 17) {
      if let loginCheck = loginList.object(forKey: Text.email.text!) as? String {
        if loginCheck == Text.password.text! {
          let afterLogin = AfterLoginViewController()
            afterLogin.emailText = Text.email.text ?? " "
      ...
      let textCheckAlert = UIAlertController (title: "알람", message: "글자 3초과, 17미만으로 입력하시오.", preferredStyle: .alert)
      let textCheckOk = UIAlertAction (title: "확인", style: .default)
      textCheckAlert.addAction(textCheckOk)
      present(textCheckAlert, animated: true)
      Text.email.text = ""
      Text.password.text = ""
    }
  }
  @objc func actionSignInButoon(_ sender: UIButton) {
    let SignView = SignInViewController()
    SignView.modalPresentationStyle = .fullScreen
    present(SignView, animated: true)
  }
}
